...
After upload, run the hosted health/login/dashboard, learner and Manage route inventory, content visibility, progress/history/result, Practice and mock-exam attempt, Writing, Speaking browser-local recording/fallback, security-header, protected-path, generic-error, and mobile overflow checks. If provider-edge blocking prevents a direct `/health` navigation, record that limitation and verify the same-origin response without weakening source-path protections.
...
## Sprint 9 production update

Sprint 9 batch 1 is content-only. The reviewed [Sprint 9 SQL](../database/infinityfree/sprint-9-update.sql) adds original topics, vocabulary, grammar, Reading, draft Listening, Writing, and Speaking content through guarded inserts. It contains no `CREATE TABLE`, `ALTER TABLE`, `DROP`, `TRUNCATE`, destructive `DELETE`, database recreation, or migration-row insert. Production must remain at exactly 20 tables and 19 migrations after import.

Before import, export the existing production database and record the complete Sprint 8 baseline, especially vocabulary progress, topics, vocabulary, grammar, Reading, Listening, questions/options, writing history, and speaking history. Apply the SQL once in phpMyAdmin. Its guards make a retry a no-op, but a second import is not part of the deployment procedure. Verify 13 topics, 35 vocabulary entries, 5 grammar lessons, 5 passages, 3 listening records, 14 questions, 45 options, 5 exercises, 9 exercise links, 6 writing prompts, and 9 speaking prompts. Existing progress and all pre-Sprint 9 records must retain their values.
