...
The deployed Sprint 3 pages are the Practice catalog, exercise detail/start page, attempt page, result page, and Manage Exercise list/create/show/edit/preview views. The hosted pilot displayed the prompt and choices without the answer key, showed the save/timer state, and revealed correctness/explanation only on the submitted result. The pages passed the hosted desktop and 360px responsive overflow check. History, wrong-answer review, Exam, Writing, and Speaking page rows below remain the approved future design, not shipped Sprint 3 features.
