...
The full workflow, SQL import method, future forward-only schema strategy, persistence behavior, and rollback procedure are in [docs/15-deployment-plan.md](docs/15-deployment-plan.md). Sprint 11 vocabulary spaced review is deployed and hosted-verified on InfinityFree; Render + Neon remains an inactive fallback requiring separate approval.
